...
bool BlockBuf<TAllocator,MaxBlockNum>::increase_capacity(size_t increate_size)
{
    ...
    {
        NET_LOG("increase capacity failed!!! newBlockNum/mMAXBLOCKNUM=",newBlockNum, mMAXBLOCKNUM);
        return false;
    }
...
bool BlockBuf<TAllocator, MaxBlockNum>::append(const char* data, size_t len)
{
    ...
    {
        NET_LOG("append failed!!! not enough buffer and increase failed");
        return false;
    }
}
...
int BlockBuf<TAllocator, MaxBlockNum>::read(SOCKET s, sockaddr_in* pAddr/*=NULL*/, int soType/*=SOCK_STREAM*/)
{
    ...
    else
    {
        ...
        NET_LOG("Blockbuf::read, read nothing, lastErrCode=",uLastErr);
    }
    return ret;
}
...
int BlockBuf<TAllocator, MaxBlockNum>::write(SOCKET s, const char* data, size_t len, sockaddr_in* pAddr/*=NULL*/, int soType/*=SOCK_STREAM*/)
{
    ...
    if(nsent < 0)
    {
        ...
        NET_LOG("socket was blocked, Async send data, LastErrCode=",uLastError);
        if (uLastError == WSAEWOULDBLOCK || uLastError == WSAEINTR || uLastError == WSAEINPROGRESS)
            nsent = 0;
		else
        {
            nsent = 0;
			NET_LOG("WSAGetLastError != WSAEWOULDBLOCK, link maybe broken! lastErrCode=",uLastError);
        }
#else
    ...
    {
        NET_LOG("write, append failed!!! send data len=",len);
    }
    return (int)nsent;
}
