...
{
    memset(m_userName, 0, ARRY_LEN);
    memset(m_passWd, 0, ARRY_LEN);
}
...
int LinkLayerProxy::init(NetMod::Extension* e)
{
    ...
    m_uProxyIp   = ext->Ip;
    strncpy(m_userName, ext->username, ARRY_LEN);
    strncpy(m_passWd, ext->passwd, ARRY_LEN);
...
int LinkLayerProxy::onConnected()
{
    ...
    m_pOwner->notifyConnState(CNetEventConnState::CS_TRANSPTLAYER_CONNECTED);
