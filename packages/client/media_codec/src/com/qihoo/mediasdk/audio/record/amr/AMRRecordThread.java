...
public class AMRRecordThread extends Thread {
	...
	public AMRRecordThread(int seq, IRecordListener listener, LocalSocket socket) {
	...
	public void run() {		
		try {
			...
			if( mListener != null ) {
				mListener.onFrame(mHeader, 0, len);
			}
				...
				while(true) {
					...
					if( frame_len == 0 ) {
						...
						if( mBuffer[0] != mConfig.getByte0() ) {
							...
							if( mListener != null ) {
								mListener.onFrame(mBuffer, 0, mBuffer.length);
							}
						}
						mStart = mBKBufer.position();
					} else {
