...
void	NetLink::connect() {
	LOG(TAG_MGROUP, "connect %s, ip/port=%s, %d", m_strName.c_str(), m_strIp.c_str(), m_nPort);
...
void	NetLink::startReconn() {
	LOG(TAG_MGROUP, "start reconnect %s.", m_strName.c_str() );
	timeval tm;
...
void	NetLink::read_cb(struct bufferevent *bev, void *ctx)
{
	...
	while(true) {
		...
		if( len <= 4 ) {
			LOG(TAG_MGROUP, "NetLoop::read_cb, len<=4");	
			break;
		}
		...
		if( msg_len <= len-4 ) {
			...
			link->m_pLooper->getMGMgr()->getHandler()->handle(bufferevent_getfd(bev), link->m_pLooper->getBuffer(), msg_len);						
			if( msg_len == len ) {
				break;
			}
		} else {
...
void	NetLink::event_cb(bufferevent *bev, short events, void *user_data) 
{
	...
	if (events & BEV_EVENT_ERROR | events & BEV_EVENT_EOF ) {
		LOG(TAG_MGROUP, "disconnected %s, linkid=%d", link->m_strName.c_str(), linkid);
		link->close();
		link->stopReconn();
		link->startReconn();
	} else if( events&BEV_EVENT_CONNECTED ) {
		LOG(TAG_MGROUP, "connected %s, linkid=%d", link->m_strName.c_str(), linkid);
		link->stopReconn();
	}
}
