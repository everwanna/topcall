...
void	NetListener::listener_cb(evconnlistener *listener, evutil_socket_t fd, sockaddr *sa, int socklen, void *user_data)
{
	...
	if (!bev) {
		LOG(TAG_LTABLE, "NetLoop::listener_cb, fail on bufferevent_socket_new.");
	...
	LOG(TAG_LTABLE, "connect from linkid=%d, ip/port=%s, %d", linkid, inet_ntoa(addr->sin_addr), addr->sin_port);
}
...
void	NetListener::read_cb(struct bufferevent *bev, void *ctx)
{
	...
	while(true) {
		...
		if( len <= 4 ) {
			LOG(TAG_LTABLE, "NetLoop::read_cb, len<=4, len=%d", len);	
			break;
		}
...
void	NetListener::event_cb(bufferevent *bev, short events, void *user_data) 
{
	...
	if (events & BEV_EVENT_EOF || events & BEV_EVENT_ERROR) {
		LOG(TAG_LTABLE, "disconnected, linkid=%d", linkid);
		listener->m_pLooper->getLTMgr()->getLinkMgr()->remove(linkid);
		listener->m_pLooper->getLTMgr()->getSlaveMgr()->remove(linkid);

	} else if ( events & BEV_EVENT_CONNECTED ) {
		LOG(TAG_LTABLE, "connected, linkid=%d", linkid);		
	}
}
