...
void	NetLink::connect() {
	LOG(TAG_LTABLE, "connect %s, ip/port=%s, %d", m_strName.c_str(), m_strIp.c_str(), m_nPort);
	...
	LOG(TAG_LTABLE, "connect %s, linkid=%d", m_strName.c_str(), bufferevent_getfd(m_pEvent) );
}
...
void	NetLink::startReconn() {
	LOG(TAG_LTABLE, "start reconnect %s.", m_strName.c_str() );
	timeval tm;
...
void	NetLink::read_cb(struct bufferevent *bev, void *ctx)
{
	...
	while(true) {
		...
		if( len <= 4 ) {
			LOG(TAG_LTABLE, "NetLink::read_cb, len<=4");	
			break;
		}
		...
		if( msg_len <= len ) {
			...
			LOG(TAG_LTABLE, "NetLoop::read_cb, not enough data, waiting. msg_len=%d, len=%d", msg_len, len);	
			break;
		}
	}
}
...
void	NetLink::event_cb(bufferevent *bev, short events, void *user_data) 
{
	...
	if (events & BEV_EVENT_ERROR || events & BEV_EVENT_EOF ) {
		LOG(TAG_LTABLE, "%s disconnected, linkid=%d", link->m_strName.c_str(), linkid);
		link->close();
		link->stopReconn();
		link->startReconn();
	} else if( events&BEV_EVENT_CONNECTED ) {
		LOG(TAG_LTABLE, "%s connected, linkid=%d", link->m_strName.c_str(), linkid);
		link->stopReconn();

		//this must be a slave link:
		PLTRegSlaveReq req;
		req.name = link->m_pLooper->getLTMgr()->getConfig()->name;
		Pack pk(SVID_LOADTABLE, PLTRegSlaveReq::uri);
		req.marshall(pk);
		pk.pack();
