...
void	Store::subscribe(Consumer* consumer, const std::string& topic) {
	if( !hasTopic(topic) ) {
		LOG(TAG_PULL, "Store::subscribe, topic %d doesn't exist", topic);
		return;
	}

	for( std::list<Consumer*>::iterator it = m_mapTopics[topic].begin(); it != m_mapTopics[topic].end(); it++ ) {
		if( (*it)->getLinkId() == consumer->getLinkId() ) {
			LOG(TAG_PULL, "Store::subscribe, consumer %s already subscribe to topic %d", consumer->getName().c_str(), topic);
			return;
		}
	}
...
void	Store::unsubscribe(const std::string& name) {
	...
	for( it1 = m_mapTopics.begin(); it1 != m_mapTopics.end(); it1++ ) {
		...
		for( std::list<Consumer*>::iterator it2 = it1->second.begin(); it2 != it1->second.end(); it2++ ) {
			if( (*it2)->getName() == name ) {
				LOG(TAG_PULL, "Store::unsubscribe, remove consumer %s from topic ", name.c_str(), topic);
...
void	Store::unsubscribe(const std::string& name, const std::string& topic) {
	...
	LOG(TAG_PULL, "Store::subscribe, consumer %s for topic %d doesn't exist", name.c_str(), topic);
}

void	Store::push(const std::string& topic, const char* data, int len) {
	if( !hasTopic(topic) ) {
		LOG(TAG_PULL, "Store::push, topic %s not found.", topic);
		return;
	}
